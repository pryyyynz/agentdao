-- Migration 005: Add Milestones Support
...
ADD COLUMN IF NOT EXISTS current_milestone INTEGER DEFAULT 1,
ADD COLUMN IF NOT EXISTS milestones_payment_model VARCHAR(20) DEFAULT 'sequential' 
    CHECK (milestones_payment_model IN ('sequential', 'parallel'));

COMMENT ON COLUMN grants.has_milestones IS 'Whether this grant uses milestone-based payments';
COMMENT ON COLUMN grants.total_milestones IS 'Total number of milestones for this grant';
COMMENT ON COLUMN grants.current_milestone IS 'Current active milestone number';
COMMENT ON COLUMN grants.milestones_payment_model IS 'Payment model: sequential (default) or parallel';
...
CREATE TABLE IF NOT EXISTS milestones (
    ...
    status VARCHAR(50) DEFAULT 'pending' CHECK (
        status IN (
            ...
            'active',            -- Current milestone in progress
            'submitted',         -- Grantee submitted proof of work
            'under_review',      -- Agents reviewing submission
            'approved',          -- Approved, payment released
            'rejected',          -- Submission rejected
            'revision_requested' -- Needs more work
        )
    ),
    
    -- Submission
    proof_of_work_url TEXT,        -- GitHub PR, demo video, etc.
    proof_of_work_ipfs VARCHAR(100), -- IPFS hash for proof documents
    ...
    reviewed_by VARCHAR(100),      -- Agent name or admin
    
    -- Blockchain
    payment_tx_hash VARCHAR(66),
    payment_released_at TIMESTAMP,
    on_chain_milestone_id BIGINT,  -- Reference to smart contract milestone
...
COMMENT ON TABLE milestones IS 'Milestone-based payment tracking for grants';
COMMENT ON COLUMN milestones.milestone_number IS 'Sequential number of milestone (1, 2, 3, ...)';
COMMENT ON COLUMN milestones.deliverables IS 'Array of deliverable descriptions';
COMMENT ON COLUMN milestones.status IS 'Current status of milestone in review/payment cycle';
COMMENT ON COLUMN milestones.proof_of_work_url IS 'URL to proof of work (GitHub, demo, docs, etc.)';
COMMENT ON COLUMN milestones.payment_tx_hash IS 'Blockchain transaction hash for milestone payment';
...
-- ============================================================================
-- STEP 4: Add milestone activity types to agent_activity_log
-- ============================================================================

-- Update the activity_type check constraint to include milestone events
...
-- ============================================================================
-- STEP 5: Create helper functions for milestone management
-- ============================================================================

-- Function to activate next milestone after current one is approved
...
BEGIN
    ...
    IF NEW.status = 'approved' AND OLD.status != 'approved' THEN
        ...
        IF grant_has_milestones AND payment_model = 'sequential' THEN
            ...
            -- Update current milestone in grants table
            IF FOUND THEN
                UPDATE grants 
                ...
                INSERT INTO agent_activity_log (
                    ...
                    jsonb_build_object(
                        'milestone_number', next_milestone_num,
                        'previous_milestone', NEW.milestone_number
                    ),
                    TRUE
                );
            ELSE
                -- All milestones completed, update grant status
                UPDATE grants 
                ...
                INSERT INTO agent_activity_log (
                    ...
                    jsonb_build_object(
                        'total_milestones', NEW.milestone_number
                    ),
                    TRUE
                );
            END IF;
...
-- Create trigger for milestone logging
...
-- ============================================================================
-- STEP 6: Create views for milestone analytics
-- ============================================================================

-- View for milestone progress summary
...
COMMENT ON VIEW milestone_progress_summary IS 'Summary view of milestone progress for all grants';
